...
[X, K, N] = si.initial_projection(n, M1); 
...
for i = 0: num_steps_sim-1
    ...
    if(trsec > currsec)
        ...
        if(intc >= X(x_c) && intc < X(1))
            ...
            network_links.(linkStr).gridN1(n+1,i+1) = N(l) + K(l)*(X(l)-intc);
        elseif(intc < X(x_c))
            network_links.(linkStr).gridN1(n+1,i+1) = N(x_c) + si.k_c(trsec)*(sim_links.(linkStr).v(trsec)*((i+1)*T - sim_links.(linkStr).Sw(trsec-1)*T) - network_links.(linkStr).chi + X(x_c));
        else
            network_links.(linkStr).gridN1(n+1,i+1) = NaN;
        end
    elseif(trsec == currsec)
        intc = (i+1)*T - network_links.(linkStr).chi / sim_links.(linkStr).v(trsec);
        if(intc > n*T && intc <= (n+1)*T)
            network_links.(linkStr).gridN1(n+1,i+1) = M1 + network_links.(linkStr).UBC(n+1)*(intc - n*T);
        elseif(intc > (n+1)*T)
            network_links.(linkStr).gridN1(n+1,i+1) = M1 + T * network_links.(linkStr).UBC(n+1) + si.k_c(trsec)*(sim_links.(linkStr).v(trsec)*((i+1)*T-(n+1)*T) - network_links.(linkStr).chi + network_links.(linkStr).xi);
        else
            network_links.(linkStr).gridN1(n+1,i+1) = NaN;
        end
    else
...
for i = 0: num_steps_sim-1
    ...
    if(trsec > currsec)
        ...
        if(intc <= X(x_c) && intc > X(length(X)))
            ...
            network_links.(linkStr).gridN2(n+1,i+1) = N(u) + K(u-1)*(X(u)-intc) + si.k_c(trsec)*((i+1)*T - sim_links.(linkStr).Sw(trsec-1)*T)*(sim_links.(linkStr).v(trsec) - w);   
        elseif(intc > X(x_c))
            network_links.(linkStr).gridN2(n+1,i+1) = N(x_c) + si.k_c(trsec)*(sim_links.(linkStr).v(trsec)*((i+1)*T - sim_links.(linkStr).Sw(trsec-1)*T) - network_links.(linkStr).xi + X(x_c));
        else
            network_links.(linkStr).gridN2(n+1,i+1) = NaN;
        end
    elseif(trsec == currsec)
        intc = (i+1)*T;
        if(intc > n*T && intc <= (n+1)*T)
            network_links.(linkStr).gridN2(n+1,i+1) = M1 + network_links.(linkStr).UBC(n+1)*(intc - n*T);
        elseif(intc > (n+1)*T)
            network_links.(linkStr).gridN2(n+1,i+1) = M1 + T * network_links.(linkStr).UBC(n+1) + si.k_c(trsec)*(sim_links.(linkStr).v(trsec)*((i+1)*T-(n+1)*T) - 0);
        else
            network_links.(linkStr).gridN2(n+1,i+1) = NaN;
        end
    else
